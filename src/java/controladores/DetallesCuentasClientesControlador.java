...
import modelos.CuentasClientesDetalle;
...
public class DetallesCuentasClientesControlador {
    ...
    public static boolean agregarcuentadetalle(CuentasClientesDetalle cuentasclientesdetalle){
        ...
        if (Conexion.conectar()){
            try {
                String sql = "select * from cuentas c left join factura_ventas fv on fv.id_factura_venta=c.id_factura_venta where c.id_cuenta='" + cuentasclientesdetalle.getCuentasclientes().getId_cuenta() +"'";
                    ...
                    while (rs.next()) {
                        ...
                        for (int i = 0; i < totalcuota; i++) {
                          ...
                          //  nrocuota = nrocuota+1;
                            ...
                            //cuotas 
                            String sqli = "insert into detallescuentas (id_cuenta, nro_cuota, vencimiento_cuota, monto_cuota, estado_cuota)"
    ...
    public static CuentasClientes buscarId1(CuentasClientes cuentasclientes) {
        if (Conexion.conectar()){
            String sql = "select * from cuentas ct, factura_ventas f, clientes c where id_cuenta='"+cuentasclientes.getId_cuenta()+"' and f.id_tipo_factura=2 and f.id_factura_venta=ct.id_factura_venta and estado_cuenta='ACTIVO'";
    ...
    public static String buscarNombre(String nombre, int pagina) {
        ...
        if (Conexion.conectar()) {
            ...
            try {
                  ...
                  String sql = "select * from cuentas ct, factura_ventas f, clientes c, tipo_facturas tf where f.id_factura_venta=ct.id_factura_venta and tf.nombre_tipo_factura= 'CREDITO' and estado_cuenta='ACTIVO' and f.id_cliente=c.id_cliente and ci_cliente like '%"+nombre+"%'"
                /*String sql = "select * from cuentas where upper(estado_cuenta) like '%" +
                        nombre.toUpperCase() + "%'";*/
