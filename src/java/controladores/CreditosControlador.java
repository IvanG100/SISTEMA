...
public class CreditosControlador {
    ...
    public static boolean modificar(Creditos aprobacioncredito){
        ...
        if (Conexion.conectar()){ 
            ...
            String sql = "update aprobacion_creditos set fecha_aprobacioncredito='" + v1 + "', "
                    ...
                    + "limite_credito='" + v6 + "'"
                    + " where id_aprobacioncredito=" + aprobacioncredito.getId_aprobacioncredito();
    ...
    public static boolean eliminar(Creditos aprobacioncredito){
        boolean valor = false;
        if (Conexion.conectar()){
            String sql = "delete from aprobacion_creditos where id_aprobacioncredito=" + aprobacioncredito.getId_aprobacioncredito();
    ...
    public static Creditos buscarId(Creditos aprobacioncredito) {
        if (Conexion.conectar()){
            String sql = "select * from aprobacion_creditos where id_aprobacioncredito ='"+aprobacioncredito.getId_aprobacioncredito()+"'";
            
            try {
                ResultSet rs = Conexion.getSt().executeQuery(sql);
                if (rs.next()){
                    aprobacioncredito.setId_aprobacioncredito(rs.getInt("id_aprobacioncredito"));
                    ...
                    aprobacioncredito.setId_aprobacioncredito(0);
        ...
        if (Conexion.conectar()) {
            ...
            try {
                ...
                String sql = "select * from aprobacion_creditos where upper(estado_aprobacioncredito) like '%" +
                        nombre.toUpperCase() + "%'"
                        + "order by id_aprobacioncredito offset " + offset + " limit " + Utiles.REGISTROS_PAGINA;
                ...
                try (PreparedStatement ps = Conexion.getConn().prepareStatement(sql)) {
                    ...
                    while (rs.next()) {
                        tabla += "<tr>"
                                + "<td>" + rs.getString("id_aprobacioncredito") + "</td>"
        ...
        if (Conexion.conectar()){
            ...
            try {
                ...
                if (rs.next()){
                    ...
                    aprobacioncredito.setId_aprobacioncredito(1);
