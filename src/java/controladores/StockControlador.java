...
public class StockControlador {
    public static boolean agregar(Stock stock) {
        ...
        if (Conexion.conectar()) {
            ...
            try {
                ...
                if (!(rs.next())) {
                    String sql2 = "insert into stock(id_articulo, cantidad_stock, id_ubicacion)"
                            + "values('" + stock.getArticulo().getId_articulo() + "',"
                            + "'" + stock.getCantidad_stock() + "',"
                            + "'" + stock.getUbicacion().getId_ubicacion() + "')";
    ...
    public static boolean descontar(Stock stock){
        ...
        if (Conexion.conectar()){ 
            String sql = "select * from stock where "
                    + "id_articulo='" + stock.getArticulo().getId_articulo() + "'" + " and cantidad_stock >= '" + stock.getCantidad_stock()+ "' ";
            ...
            try {
                ...
                if ((rs.next())) {
                    ...
                    + " where id_articulo=" + stock.getArticulo().getId_articulo();
    ...
    /*public static boolean descontar(Stock stock){
     ...
     public static boolean sumarAjusteStock(Stock stock) {
        boolean valor = false;
        if (Conexion.conectar()) {
            String sql = "select * from ajuste_stock_detalle where id_ajuste_stock='" + stock.getId_stock() + "'";
                ...
                try (PreparedStatement ps = Conexion.getConn().prepareStatement(sql)) {
                    ...
                    while (rs.next()) {
                        String sql1 = "update stock set cantidad_stock = cantidad_stock + '" + rs.getInt("cantidad_ajuste_stock") + "' "
                            + " where id_articulo=" + rs.getInt("id_articulo") + "";
     ...
     public static boolean sumarStock(Stock stock) {
        boolean valor = false;
        if (Conexion.conectar()) {
            String sql = "select * from factura_detalle_ventas where id_factura_venta='" + stock.getId_stock() + "'";
            /*String sql = "select * from ajustes_stock sa, ajuste_stock_detalle asd"
                    + " where sa.id_ajuste_stock=asd.id_ajuste_stock and sa.id_ajuste_stock=1";*/
